...
def attempt_install_pip(module_name, message):
    ...
    try:
        ...
        bot.reply_to(
            message,
            f"{ce('cross')} Failed to install <code>{package_name}</code>:\n<code>{(result.stderr or result.stdout)[:2000]}</code>",
            parse_mode="HTML",
        )
        return False
    except Exception as e:
...
def broadcast_to_hosted_bots(message_text, admin_chat_id):
    results = []
    running_scripts = [(sk, si) for sk, si in bot_scripts.items() if is_bot_running(si["script_owner_id"], si["file_name"])]
...
def send_to_process_init(message):
    ...
    user_running = [
        (sk, si)
        for sk, si in bot_scripts.items()
        if (user_id == si["script_owner_id"] or user_id in admin_ids) and is_bot_running(si["script_owner_id"], si["file_name"])
    ]
    if not user_running:
...
def view_all_logs(message):
    ...
    for log_file, size, _ in sorted(user_logs):
        size_kb = size / 1024
        markup.add(types.InlineKeyboardButton(f"📄 {log_file} ({size_kb:.1f} KB)", callback_data=f"viewlog_{user_id}_{log_file}"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="send_command"))
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(types.InlineKeyboardButton(f"{status_icon} {file_name} [{file_type.upper()}]", callback_data=f"file_{user_id}_{file_name}"))
...
def _logic_statistics(message):
    ...
    for script_key_iter, script_info_iter in list(bot_scripts.items()):
        ...
        if len(parts) == 2:
            script_owner = int(parts[0])
            if is_bot_running(script_owner, script_info_iter["file_name"]):
                running_bots_count += 1
                if script_owner == user_id:
                    user_running_bots += 1
...
def _logic_system_stats(chat_id):
    try:
        ...
        running_scripts = sum(1 for _, si in bot_scripts.items() if is_bot_running(si["script_owner_id"], si["file_name"]))
        stats = (
...
def logs_bot_callback(call):
    try:
        ...
        try:
            ...
            bot.send_message(chat_id, f"📜 <b>Logs:</b> <code>{file_name}</code>\n<pre>{log_content}</pre>", parse_mode="HTML")
        except Exception:
...
def broadcast_bots_callback(call):
    bot.answer_callback_query(call.id)
    running_count = sum(1 for _, si in bot_scripts.items() if is_bot_running(si["script_owner_id"], si["file_name"]))
    msg = bot.send_message(
        call.message.chat.id,
        f"{ce('broadcast')} <b>Broadcast to Hosted Bots</b>\n\n"
...
def handle_confirm_broadcast(call):
    ...
    try:
        ...
        threading.Thread(
            ...
            args=(broadcast_text, broadcast_photo_id, broadcast_video_id, caption, chat_id),
            daemon=True,
        ).start()
    except ValueError as e:
        bot.edit_message_text(f"{ce('cross')} Error: {e}", chat_id, call.message.message_id, reply_markup=None, parse_mode="HTML")
    except Exception as e:
...
def admin_panel_callback(call):
    user_id = call.from_user.id
    bot.answer_callback_query(call.id)
    running_bots_count = sum(1 for _, si in bot_scripts.items() if is_bot_running(si["script_owner_id"], si["file_name"]))
...
if __name__ == "__main__":
    ...
    logger.info("🛡️ Admins: %s", admin_ids)
